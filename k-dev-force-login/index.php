...
function kdfl_settings_of_plugin(){
	if(isset($_POST) && isset($_POST['kdfl-link'])){
		update_option("kdfl-link",$_POST['kdfl-link']);
		...
		<form method='post'>
			<div class='kdfl-settings' style='display:flex;margin-top: 1rem;'>
				...
				<div class='kdfl-des' style='width: 80%;'>
					<input type='text' name='kdfl-link' placeholder='<?php echo __('Login URL','k-dev-force-login'); ?>' style='width: 100%;' <?php 
...
function kdfl_head(){
	if(get_option("kdfl-link","Na/N") !== "Na/N"){
		header("location: ".get_option("kdfl-link"));
		exit();
	}
}
?>